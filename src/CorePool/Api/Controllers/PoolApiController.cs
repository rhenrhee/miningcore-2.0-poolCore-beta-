...
using CorePool.Api.Extensions;
using CorePool.Api.Responses;
    ...
    public class PoolApiController : ControllerBase
    {
            ...
            [FromQuery(Name = "i")] SampleInterval interval = SampleInterval.Minute)
        {
            ...
            switch(range)
            {
                ...
                default:
                    throw new ApiException("неверный интервал");
            }
            ...
            string poolId, string address, [FromQuery] SampleRange perfMode = SampleRange.Day)
        {
            ...
            if(string.IsNullOrEmpty(address))
                throw new ApiException($"Некорректный или не существующий адрес майнера", HttpStatusCode.NotFound);
            ...
            string poolId, string address, [FromQuery] int page, [FromQuery] int pageSize = 15)
        {
            ...
            if(string.IsNullOrEmpty(address))
                throw new ApiException($"Неверный или не существующий адрес майнера", HttpStatusCode.NotFound);
        ...
        public async Task<Responses.WorkerPerformanceStatsContainer[]> GetMinerPerformanceAsync(
            string poolId, string address, [FromQuery] SampleRange mode = SampleRange.Minute)
        {
            var pool = GetPool(poolId);

            if(string.IsNullOrEmpty(address))
                throw new ApiException($"Invalid or nonexistent miner address", HttpStatusCode.NotFound);
        ...
        private PoolConfig GetPool(string poolId)
        {
            ...
            if(pool == null)
                throw new ApiException($"Pool {poolId} unknown", HttpStatusCode.NotFound);
