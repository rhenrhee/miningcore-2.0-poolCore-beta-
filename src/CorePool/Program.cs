...
using CorePool.Api;
using CorePool.Api.Controllers;
...
using CorePool.Persistence.Dummy;
using CorePool.Persistence.Postgres;
using CorePool.Persistence.Postgres.Repositories;
using CorePool.Util;
...
using CorePool.Api.Middlewares;
...
namespace CorePool
{
    public class Program
    {
        ...
        private static bool HandleCommandLineOptions(string[] args, out string configFile)
        {
            ...
            {
                FullName = "PoolCore - Pool Mining Engine",
        ...
        private static void ValidateRuntimeEnvironment()
        {
            ...
            if(RuntimeInformation.IsOSPlatform(OSPlatform.Windows) && RuntimeInformation.ProcessArchitecture == Architecture.X86)
                throw new PoolStartupAbortException("CorePool requires 64-Bit Windows");
        }
        ...
        private static void StartApi()
        {
            ...
            logger.Info(() => $"API has runnig Online @ {address}:{port}{(!enableApiRateLimiting ? " [rate-limiting disabled]" : string.Empty)}");
